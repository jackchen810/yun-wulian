...
class RunLogsHandle {
    ...
    async list(req, res, next) {
        ...
        //参数有效性检查
        let queryList = await DB.DevunitRunLogsTable.find(filter).sort(sort).exec();
    ...
    async page_list(req, res, next) {
        ...
        let total = await DB.DevunitRunLogsTable.count(filter);
        let skipnum = (current_page - 1) * page_size;   //跳过数
        let queryList = await DB.DevunitRunLogsTable.find(filter).sort(sort).skip(skipnum).limit(page_size).exec();
    ...
    async prj_list(req, res, next) {
        ...
        logger.info('run project logs list');
        //logger.info(req.body);
        ...
        //参数有效性检查
        let logList = await DB.DevunitRunLogsTable.find(wherestr).exec();
        if (logList.length == 0) {
            res.send({ret_code: -1, ret_msg: '数据不存在', extra: ''});
            return;
        }
        ...
        logger.info('run project logs list end');
    }


    async prj_page_list(req, res, next) {

        logger.info('run project page logs list');
        //logger.info(req.body);
        ...
        let total = await DB.DevunitRunLogsTable.count(wherestr);
        let skipnum = (current_page - 1) * page_size;   //跳过数
        let logList = await DB.DevunitRunLogsTable.find(wherestr).sort(sort).skip(skipnum).limit(page_size).exec();
        res.send({ret_code: 0, ret_msg: '成功', extra: logList, total: total});
        logger.info('run project page logs list end');
    }

}


module.exports = new RunLogsHandle();
