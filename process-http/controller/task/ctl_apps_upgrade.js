...
class ManageHnd {
	...
	try{
		...
		if(mode === '1'){
			...
			MqttPubHandle.setTaskTime(TaskTable, expired_time, exec_time);
			MqttPubHandle.setTaskAdditions(TaskTable, additions);
			await MqttPubHandle.CMD_EXE.apps(TaskTable,logo,content);
		}else if(mode === '2'){
...
/*			var apps_callback = async function(mac, jsonObj) {
			...
			var timeout_callback = async function(TaskTable) {
				logger.info('start run timeout_callback');
				logger.info('uuid='+TaskTable.uuid);
				var task = await TaskTable.find({'uuid':TaskTable.uuid ,'cmd_item':'apps'});
				for(var i=0;i < task.length;i++){
					if(task[i].additions.mode == 2 && task[i].pubsub_status == 'request'){
						await TaskTable.findOneAndUpdate({'uuid': task[i].uuid, 'mac':task[i].mac, 'pubsub_status':'request'},{$set: {'pubsub_status':'response_fail'}});
					}
			...
			setTimeout(timeout_callback,expired_time*60*60*1000,TaskTable);
		*/
		}else if(mode === '3'){
	...
	/*
			var apps_callback = async function(){
				...
				//	MqttPubHandle.setTaskTime(TaskTable, expired_time, -1);
				//	MqttPubHandle.setTaskAdditions(TaskTable, additions);
				//	await MqttPubHandle.CMD_EXE.apps(TaskTable,logo,content);
					await TaskTable.createBatchPublish(TaskTable.uuid, exec_time);
				}else {
			...
			var timeout_apps = async function(TaskTable){
			...
			setTimeout(timeout_apps,expired_time*60*60*1000,TaskTable);*/
		}
		logger.info('apps成功');
