...
class DeviceHandle {
    constructor(){
        ...
        this.onLineList = this.onLineList.bind(this);
        this.offLineList = this.offLineList.bind(this);
        this.listOnOffline = this.listOnOffline.bind(this);
    }
    ...
    async onLineList(req, res, next){
        ...
        this.listOnOffline(req, res, next, 'online');
    ...
    async offLineList(req, res, next){
        ...
        this.listOnOffline(req, res, next, 'offline');
    ...
    async batchRemove(req, res, next){
        ...
        logger.info('device batch remove');
        //logger.info(req.body);
        ...
        try {
            ...
            for (var i = 0; i < mac_array.length; i++) {
                ...
                // 更新DeviceTable, 如果升级ok的话就不更新
                var wherestr = {'mac': mac, 'user_name': user_name};

                //如果采用返回值得形式，必须的await
                var query = await DB.DeviceTable.findOneAndRemove(wherestr).exec();
                if (query == null) {
                    ill_mac.push(mac_array[i])
                }
            }
        ...
        logger.info('device batch remove end');
    }
...
module.exports = new DeviceHandle();
